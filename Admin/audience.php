...
$sql = "SELECT Vis_Date,MONTH(Vis_Date),YEAR(Vis_Date), Dis_ID, count(Dis_ID) as userlook  FROM visit_disease
WHERE MONTH(Vis_Date) LIKE  '%".$month."%' OR YEAR(Vis_Date) LIKE '%".$year."%' GROUP BY Dis_ID ORDER BY userlook DESC ";
...
for ($i = 0 ; $i < $n ;$i++){
  ...
  array_push($arraynamelook2,$result["Dis_ID"]);
}
// for($x = 0; $x < $n; $x++) {
...
for ($i = 0; $i < $n ;$i++){
  $sql1= "SELECT * from disease WHERE Dis_ID Like $arraynamelook2[$i]";
  $query1 = mysqli_query($link,$sql1);
  $result1 = mysqli_fetch_array($query1,MYSQLI_ASSOC);
array_push($arraynamelook,$result1["Dis_Name"]);
    }
    if(!$result&&!$result1)
    {
        echo '<script>';
        echo "alert('no Have !');";
        echo 'window.location="audience.php";';
        echo '</script>';
    }
                        ...
                        <div class="bg-faded  rounded p-5">
                          <form  action="audience.php" method="post">
                            <h2 class="section-heading mb-4">
                                <span class="section-heading-lower">รายงานชื่อโรคที่ถูกเข้าชมมากที่สุด</span>
                            </h2>
                            <div class="row ">
                                <div class="col-sm-3">
                                    <p>ประจำเดือน </p>
                                ...
                                <div class="col-sm-9">
                                    <div class="select-wrapper">
                                      <select name="year"  class="form-control">
                                        <option>- เลือก -</option>
                                        <?php
                                        $sql3 = "SELECT DISTINCT YEAR(Vis_Date) FROM  visit_disease ORDER by YEAR(Vis_Date) DESC";
                                        ...
                                        while($result3 = mysqli_fetch_array($query3,MYSQLI_ASSOC))
                                        {
                                         ...
                                         ?>
                                               ...
                                               <option value="<?php echo $result3["YEAR(Vis_Date)"];?>"><label class="form-control" ><?php echo $result3["YEAR(Vis_Date)"];?></label ></option>
                                    ...
                                    <table class="table table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">จำนวนผู้เข้าชม</th>
                                        ...
                                        <tbody>
                                            <tr>
                                                <th scope="row">1</th>
                                                <td><?php echo $arraynamelook[0]; ?></td>
                                                <td><?php echo $arrayUserlook[0]; ?></td>

                                            </tr>
                                            <tr>
                                                <th scope="row">2</th>
                                                <td><?php echo $arraynamelook[1]; ?></td>
                                                <td><?php echo $arrayUserlook[1]; ?></td>

                                            </tr>
                                            <tr>
                                                <th scope="row">3</th>
                                                <td><?php echo $arraynamelook[2]; ?></td>
                                                <td><?php echo $arrayUserlook[2]; ?></td>

                                            </tr>
                                          <tr>
                                            <th scope="row">4</th>
                                            <td><?php echo $arraynamelook[3]; ?></td>
                                            <td><?php echo $arrayUserlook[3]; ?></td>

                                        </tr>
                                        <tr>
                                        <th scope="row">5</th>
                                        <td><?php echo $arraynamelook[4]; ?></td>
                                        <td><?php echo $arrayUserlook[4]; ?></td>
